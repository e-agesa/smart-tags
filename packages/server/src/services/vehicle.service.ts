...
export async function createVehicle(
  ...
  const rows = await query<Vehicle>(
    `INSERT INTO vehicles (user_id, license_plate, make, color)
     VALUES ($1, $2, $3, $4)
     RETURNING *`,
    [userId, licensePlate.toUpperCase().trim(), make || null, color || null]
  );
  return rows[0];
}
...
export async function getUserVehicles(userId: string): Promise<Vehicle[]> {
  return query<Vehicle>(
    `SELECT v.*, t.tag_code, t.status as tag_status
     FROM vehicles v
